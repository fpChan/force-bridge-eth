...
# start relayer
#${FORCE_CLI} init-ckb-light-contract -i 1 --to "${CKB_CHAIN_ADDRESS}" -f 500 -c 40000 --eth-rpc-url "${ETH_RPC_URL}" --wait
ps aux | grep 'force-eth-cli ckb-relay' | grep -v grep | awk '{print $2}' | xargs kill -9
${FORCE_CLI} ckb-relay -k privkeys/ckb2eth_relayer_key --per-amount 10 --to "${CKB_CHAIN_ADDRESS}" --eth-rpc-url "${ETH_RPC_URL}" > data/ckb-relayer.log 2>&1 &
...
${FORCE_CLI} lock-eth --ckb-recipient-address "${RECIPIENT_ADDR}" --replay-resist-outpoint "${bridge_cell_outpoint}" --to "${TOKEN_LOCKER_ADDRESS}" --amount 100 --bridge-fee "${bridge_fee}" --sudt-extra-data sudt_extra_data  --rpc-url "${ETH_RPC_URL}"  --wait > "${LOCK_ETH_PATH}"
lock_eth_hash=`cat "${LOCK_ETH_PATH}"| awk '{print $4}'`
${FORCE_CLI} mint --hash "${lock_eth_hash}" --eth-contract-address "${TOKEN_LOCKER_ADDRESS}" --cell depend_on_eth_relay  --eth-rpc-url "${ETH_RPC_URL}"
${FORCE_CLI} query-sudt-blance --addr ${RECIPIENT_ADDR} --token-addr "${ETH_ADDRESS}" --lock-contract-addr "${TOKEN_LOCKER_ADDRESS}"
${FORCE_CLI} transfer-from-ckb --ckb-privkey-path privkeys/ckb_key_recipient --burn-amount 2 --unlock-fee 1 --receive-addr 0x403A53A7Dfa7a4AB022e53FeFf11232b3140407d   --token-addr ${ETH_ADDRESS} --lock-contract-addr ${TOKEN_LOCKER_ADDRESS} --light-client-addr ${CKB_CHAIN_ADDRESS} --eth-rpc-url "${ETH_RPC_URL}" --wait

# token crosschain
#${FORCE_CLI} approve --from "${TOKEN_LOCKER_ADDRESS}" --to "${TOKEN_ADDRESS}" --rpc-url "${ETH_RPC_URL}"
